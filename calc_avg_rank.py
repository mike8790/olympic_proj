...
for i in range(len(df)):
    if df.iloc[i,1:4].isna().sum() >= 2:
        av_rank_4.append(np.nan)
    else:
        av_rank_4.append(df.iloc[i,1:4].mean())
    if df.iloc[i,1:7].isna().sum() > 2:
        av_rank_7.append(np.nan)
    else:
        av_rank_7.append(df.iloc[i,1:7].mean())

d = {'country': df['country'], 'av_rank4': av_rank_4, 'av_rank7': av_rank_7}
...
avg_rank_table.av_rank4,
avg_rank_table.av_rank7
